...
const ReviewItem: React.FC<ReviewItemProps> = ({
  ...
  return (
    <div style={styles.card}>
      <ReviewHeader user={user} profileImage={profileImage} badge={badge} badgeColor={badgeColor} />
      <ReviewStarsAndDate rating={rating} date={date} />
...
const ReviewHeader: React.FC<{
  ...
  badge?: string;
  badgeColor?: string;
}> = ({ user, profileImage, badge, badgeColor }) => (
  <div style={{ display: 'flex', alignItems: 'center', marginBottom:22 }}>
    ...
    <div style={{ marginLeft: 2, display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      {badge && (
        <span style={{ fontWeight: 400, fontSize: 10, color: 'rgb(117, 125, 134)', background: badgeColor, borderRadius: 2, padding: '2px 6px', marginLeft: 2, whiteSpace: 'nowrap' }}>{badge}</span>
      )}
    </div>
  </div>
);
...
}> = ({ onLike, onReport, likeCount = 0, style }) => (
  <div style={{ ...styles.actions, ...style }}>
    <div style={{ display: 'flex', alignItems: 'end', gap: 4, marginBottom: 2 }}>
      <button style={{ ...styles.actionBtn, paddingBottom: 0 }} onClick={onLike}>도움이 돼요</button>
    ...
    <button style={{ ...styles.actionBtn, marginBottom: 2 }} onClick={onReport}>신고하기</button>
